...
class _CreateBillScreenState extends State<CreateBillScreen> {
  ...
  Future<void> _saveAndPrint() async {
    ...
    try {
      ...
      final gst = prov.user?.gst ?? 9.0;
      final sgst = prov.user?.sgst ?? 9.0;
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Invoice created & downloaded!\n$filePath'),
  ...
  Widget build(BuildContext context) {
    ...
    final gstAmount = subtotal * (prov.user?.gst ?? 9.0) / 100.0;
    final sgstAmount = subtotal * (prov.user?.sgst ?? 9.0) / 100.0;
        ...
        child: Column(
          ...
          children: [
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    _buildSummaryRow(
                      'CGST (${prov.user?.gst ?? 9}%)',
                      gstAmount,
                    ),
                    const SizedBox(height: 8),
                    _buildSummaryRow(
                      'SGST (${prov.user?.sgst ?? 9}%)',
                      sgstAmount,
                    ),
            ...
            SizedBox(
              ...
              height: 56,
              child: ElevatedButton.icon(
                onPressed: _loading ? null : _saveAndPrint,
                icon: _loading
                    ? const SizedBox(
                        height: 20,
                        width: 20,
                ...
                label: Text(_loading ? 'Creating...' : 'Save & Generate PDF'),
                style: ElevatedButton.styleFrom(
                  backgroundColor: AppTheme.primaryOrange,
                ),
              ),
